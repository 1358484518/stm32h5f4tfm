 ...
 *
 * STM production OTP backend:
 *   - HUK / IAK / boot_seed / implementation_id  <- on-chip Flash OTP (0x08FFF000)
 *   - ROTPK / LCS / NV counters / rest            <- flash-emulated OTP (0x0C028000)
 *
 * Based on platform/ext/common/template/otp_flash.c
 */
                                      ...
                                      size_t out_len, uint8_t *out)
{
    ...
    case PLAT_OTP_ID_BOOT_SEED:
        return stm_chip_otp_secrets_read_boot_seed(out, out_len);
    ...
    case PLAT_OTP_ID_IMPLEMENTATION_ID:
        return stm_chip_otp_secrets_read_impl_id(out, out_len);
                                       ...
                                       size_t in_len, const uint8_t *in)
{
    ...
    case PLAT_OTP_ID_HUK:
    case PLAT_OTP_ID_IAK:
        /* Device secrets are programmed into on-chip OTP by the factory tool. */
        (void)in_len;
        (void)in;
        return TFM_PLAT_ERR_UNSUPPORTED;
    case PLAT_OTP_ID_IAK_LEN:
